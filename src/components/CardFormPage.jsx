...
import "../styles/CardFormPage.css";
...
const CardFormPage = () => {
  ...
  return (
    <div className="cardform-container-page">
      <div className="faq-section-page">
        <h2>Frequently Asked Questions</h2>
        <ul className="faq-list-page">
          {questions.map((item, index) => (
            <li key={index} className="faq-item-page">
              <div
                className="faq-question-page"
                onClick={() => toggleAnswer(index)}
              >
                <span>{item.question}</span>
                <span className="faq-symbol-page">
                  {activeIndex === index ? "-" : "+"}
                </span>
              </div>
              {activeIndex === index && (
                <div className="faq-answer-page">{item.answer}</div>
              )}
            </li>
          ))}
        </ul>
      ...
      <div className="contact-us-page">
        <h2>Contact Us</h2>
        <p>India</p>
        <p className="contact-spacing-page"></p>
        <p className="contact-details-page">
